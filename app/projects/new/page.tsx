...
export default function NewProject() {
 ...
 <button onClick={analyze} disabled={loading || (!address && (!lat || !lon)) || !width || !depth}>{loading?'Resolving…':'Resolve site & create project'} <ArrowRight size={16}/></button>
 {error&&<div className="result"><div className="wide"><span>Error</span><strong>{error}</strong></div></div>}
 {result&&<div className="result"><div><span>Jurisdiction</span><strong>{result.jurisdiction}</strong></div><div><span>Planning source</span><strong>{result.planningSource}</strong></div><div><span>Plot area</span><strong>{result.plotArea} m²</strong></div><div className="wide"><span>Spatial evidence</span><strong>{Object.keys(result.evidence||{}).length ? 'Resolved' : 'Context only / further evidence required'}</strong></div><div className="wide"><CheckCircle2 size={16}/><span>Project created. Opening the project workspace…</span></div></div>}</section></main>;
}
